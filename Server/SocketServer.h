...
#ifndef THREADPOOLBYONLY_SOCKETSERVER_H
#define THREADPOOLBYONLY_SOCKETSERVER_H
                ...
                switch (ret) {
                    ...
                    default:
                        if (FD_ISSET(allConnection.at(i), &fdRead)) {
                            int res=dataHandler(i);
                            SOCKET so = allConnection.at(i);
                            if ( res!= -1&&res!=-2) {
                                    string ch;
                                    stringstream temp;
                                    sockSetting infor = ClientInformation.at(so);

                                    temp << infor.sin_port;
                                    ch.append("来自客户端：").append(temp.str()).append("---------->").append(revData);
                                    char mess[255]{""};
                                    ch.copy(mess, sizeof(mess), 0);
                                    *(mess + sizeof(mess)) = '\0';

                                    ch.clear();
                                for (int i = 0; i < allConnection.size(); i++) {
                                    so = allConnection.at(i);
                                    send(so, mess, 255, 0);
                                }
...
#endif //THREADPOOLBYONLY_SOCKETSERVER_H
